...
for(const page of ['index.html','portfolio.html','opportunity.html','transaction.html','notice.html']){const h=read(page);assert(h.includes('Information Memorandum'),page);assert(h.includes('rel="canonical"'),`canonical ${page}`);assert(!/repository register|release 22|SHA-256|liquidity reference|validation runner|test modules/i.test(h),`internal language ${page}`)}
...
const allowedAssets=new Set(['im.css','im.js','im-data.json','recreation-costs.json']);for(const f of fs.readdirSync(path.join(root,'assets')))assert(allowedAssets.has(f),`unexpected public asset ${f}`);
...
console.log(JSON.stringify({informationMemorandum:true,assets:67,askingReferenceEUR:11755000,rangeEUR:[9380000,14715000],recreationCostEUR:46420000,stageCounts:{developedSoftware:37,developedPrototype:25,researchStage:5},sitemapUrls:72,publicAssetFiles:[...allowedAssets]},null,2));
