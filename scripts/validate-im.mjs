...
assert(d.v==='2026.09.05-im2','version');assert(d.p[0]===67&&a.length===67,'asset count');
...
assert(sum(8)===11755000&&sum(9)===9380000&&sum(10)===14715000,'portfolio values');
...
for(const page of ['index.html','portfolio.html','opportunity.html','transaction.html','notice.html']){const h=read(page);assert(h.includes('Information Memorandum'),page);assert(h.includes('rel="canonical"'),`canonical ${page}`);assert(!/repository register|release 22|SHA-256|recreation-cost|liquidity reference|validation runner|test modules/i.test(h),`internal language ${page}`)}
for(const x of a){const p=`projects/${x[2]}/index.html`;assert(exists(p),`missing ${x[2]}`);const h=read(p);assert(h.includes('../../assets/im-data.json')&&h.includes('../../assets/im.js'),`profile ${x[2]}`)}
...
for(const x of aliases){const h=read(`projects/${x}/index.html`);assert(h.includes('noindex'),`alias ${x}`)}
...
new vm.Script(read('assets/im.js'),{filename:'assets/im.js'});
const allowedAssets=new Set(['im.css','im.js','im-data.json']);for(const f of fs.readdirSync(path.join(root,'assets')))assert(allowedAssets.has(f),`unexpected public asset ${f}`);
...
console.log(JSON.stringify({informationMemorandum:true,assets:67,askingReferenceEUR:11755000,rangeEUR:[9380000,14715000],stageCounts:{developedSoftware:37,developedPrototype:25,researchStage:5},sitemapUrls:72,publicAssetFiles:[...allowedAssets]},null,2));
