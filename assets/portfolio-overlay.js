...
function apply(){const sourceRelease=document.documentElement.dataset.release||'';if(sourceRelease!==RELEASE)swapText();document.documentElement.dataset.release=RELEASE;document.querySelectorAll('.footer-inner div').forEach(x=>{const next=x.textContent.replace(/Release 20260903-(?:16|17|18)/g,`Release ${RELEASE}`);if(next!==x.textContent)x.textContent=next})}
