...
function apply(){const sourceRelease=document.documentElement.dataset.release||'';if(sourceRelease!==RELEASE)swapText();document.documentElement.dataset.release=RELEASE;document.querySelectorAll('.footer-inner div').forEach(x=>{if(x.textContent.includes('Release 20260903-17'))x.textContent=x.textContent.replace('Release 20260903-17',`Release ${RELEASE}`);if(x.textContent.includes('Release 20260903-16'))x.textContent=x.textContent.replace('Release 20260903-16',`Release ${RELEASE}`)})}
